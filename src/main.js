let getWeather = async (city) => {
    let response = await fetch(
        `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=us&key=PJWSS4TLJKSKT2Y62AGZG9EJ5&contentType=json`,
    );
    if (!response.ok) {
        return null;
    }
...
function fmtTemp(v) {
    return metricRadio && metricRadio.checked
        ? `${toC(v)}°C`
        : `${Math.round(v)}°F`;
}
function fmtSpeed(s) {
    return metricRadio && metricRadio.checked
        ? `${Math.round(s * 1.609)} km/h`
        : `${Math.round(s)} mph`;
}
function fmtDistance(miles) {
    return metricRadio && metricRadio.checked
        ? `${(miles * 1.609).toFixed(1)} km`
        : `${miles} mi`;
}
...
function renderWeather(data) {
    ...
    if (!data) {
        ...
        if (weeklyContainer && !weeklyContainer.classList.contains("is-hidden"))
            weeklyContainer.classList.add("is-hidden");
        return;
    }
    if (!errorMessage.classList.contains("is-hidden")) {
    ...
    if (elements.weatherInfo) {
        elements.weatherInfo.onclick = () =>
            openDetailsDialog(data.days[0], data.resolvedAddress);
    ...
    if (weeklyContainer) {
        ...
        weeklyContainer.innerHTML = upcoming
            .map((day) => {
                const weekday = new Date(day.datetime).toLocaleDateString(
                    undefined,
                    { weekday: "short" },
                );
            ...
            .join("");
        if (weeklyContainer.classList.contains("is-hidden"))
            weeklyContainer.classList.remove("is-hidden");
    }
}
...
function openDetailsDialog(day, city) {
    ...
    if (dateEl)
        dateEl.textContent = new Date(day.datetime).toLocaleDateString(
            undefined,
            { weekday: "long", month: "short", day: "numeric" },
        );
    if (mainTempEl)
        mainTempEl.textContent = `High: ${fmtTemp(day.tempmax)}  •  Low: ${fmtTemp(day.tempmin)}  •  Avg: ${fmtTemp(day.temp)}`;
    ...
    if (hoursEl) {
        ...
        hoursEl.innerHTML = hours
            .map((h) => {
                const t = h.datetimeEpoch
                    ? new Date(h.datetimeEpoch * 1000).toLocaleTimeString(
                          undefined,
